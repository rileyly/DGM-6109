...
/* Sort descending by rain amount */
dataset.sort(function (a, b) {
    if (a.precipMM <= b.precipMM) {
        return 1;
    } return -1;
})
...
let maxPhotos = d3.max(dataset, function (value) { return value.photos }); // config values to experiment with x and y scaling 
let maxLocations = d3.max(dataset, function (value) { return value.locCount });
let maxRain = d3.max(dataset, function (value) { return value.precipMM }); 
...
circles.attr("r", function (value) {
    return rScale(value.precipMM);
})
    .attr("opacity", .7)
    .attr("cx", function (value) {
        return xScale(value.locCount);
    })
...
let xAxisLabel = svg.append("text")
    ...
    .text("Number of locations visited per day");
...
let xMaxLabel = svg.append("text")
    .attr("x", xScale(maxLocations))
    .attr("y", svgHeight - (bottomMargin / 1.4))
    .attr("text-anchor", "middle")
    .attr("font-family", "monospace")
    .text(String(maxLocations));
